...
public class ThreadedInputStream extends FilterInputStream {
	...
	public int read(byte[] dest, int offset, int length)
		...
		if ((offset < 0) || ((offset + length) > dest.length)) {
			...
			throw new IllegalArgumentException("offset must be at least , and "
					+ "(offset + length) must be less than or "
					+ "equal to dest.length. " + "offset=" + offset
	...
	private byte[] removeUpTo(int maxRead) throws IOException {
		...
		try {
			synchronized (buffer) {
				while (buffer.isEmpty() && !eofDetected && !ioxDetected
						&& !closeRequested) {
				...
				if (data.length > maxRead) {
					...
					System.arraycopy(data, maxRead, putBackData, 0,
							putBackData.length);
