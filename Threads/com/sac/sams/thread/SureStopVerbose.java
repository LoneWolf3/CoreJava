...
public class SureStopVerbose extends Object {
	...
	private SureStopVerbose() {
		...
		// Enough initial capacity for pending additions,
	...
	private void runWork() {
		try {
			while (true) {
				...
				print("about to sleep for . seconds");
				Thread.sleep(500);
				print("done with sleep for . seconds");
				...
				synchronized (pendingList) {
					if (pendingList.size() < 1) {
						print("about to wait on pendingList " + "for "
								+ sleepTime + " ms");
						long start = System.currentTimeMillis();
						pendingList.wait(sleepTime);
						long elapsedTime = System.currentTimeMillis() - start;
						print("waited on pendingList for " + elapsedTime
								+ " ms");
					}
					...
					if (pendingList.size() > 0) {
						...
						print("copying " + pendingList.size()
								+ " elements from pendingList to " + "stopList");
						int oldSize = stopList.size();
						stopList.addAll(pendingList);
						pendingList.clear();
						int newSize = stopList.size();
						print("pendingList.size()=" + pendingList.size()
								+ ", stopList grew by " + (newSize - oldSize));
					}
				}
			} // while
			...
			if (entry.thread.isAlive()) {
				...
				if (entry.stopTime < currTime) {
					...
					try {
						...
						System.err.println("SureStop was not permitted to "
	...
	private void addEntry(Entry entry) {
		synchronized (pendingList) {
			...
			pendingList.notify();
			print("added entry to pendingList, name=" + entry.thread.getName()
					+ ", stopTime=" + entry.stopTime + ", in "
					+ (entry.stopTime - System.currentTimeMillis()) + " ms");
		}
	}
